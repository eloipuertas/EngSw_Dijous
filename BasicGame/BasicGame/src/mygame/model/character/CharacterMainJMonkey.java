    ...
    public void personatgeUpdate() {
        ...
        if (!this.isPaused) {
            //@Ernest --  Setting limits in up/down camera movement
             if(app.getCamera().getUp().y < 0) // axis more than -0.1
             {
              // setting restriction axes  
              app.getCamera().setAxes(camLeft, new Vector3f (0,1,0), viewDirection);
                //app.getCamera().lookAt( new Vector3f(0,app.getCamera().getDirection().y,0),new Vector3f(0,0,0));
