 ...
 */
public class CharacterMainJMonkey_backup
extends AbstractAppState implements ActionListener {
    ...
    public void personatgeUpdate() {
        ...
        if (!this.isPaused) {
            if (run) {
                if (left) {
                    walkDirection.addLocal(camLeft.mult(3));
                }
                if (right) {
                    walkDirection.addLocal(camLeft.negate().mult(3));
                }
                if (up) {
                    walkDirection.addLocal(camDir.mult(3));
                }
                if (down) {
                    walkDirection.addLocal(camDir.negate().mult(3));
                }
